...
void calcOptFlow(cv::Mat Image, double img_timestamp)
{
  ...
  //printf("in = %2.4f \t out = %2.4f\n", in_features_current(0).x, out_features_current(0).x);
  //printf("distortion 1 = %2.5f\n",  cameraParams.CameraMatrix[1][2]);
  ...
  geometry_msgs::Vector3 flow_msg;
  flow_msg.x = flow_x_ang;
  flow_msg.y = flow_y_ang;
  flow_msg.z = 0.0;

  pub.publish(flow_msg);
...
void imageCallback(const sensor_msgs::ImageConstPtr& msg)
{
  ...
  try
  {
    matFrame = (cv_bridge::toCvShare(msg, "mono8"))->image;
    timestamp = (cv_bridge::toCvShare(msg, "mono8"))->header.stamp.toSec();
  }
...
int main(int argc, char **argv)
{
  ...
  if (_nh.getParam("calibration_path", calibration_path)) {
  ...
  pub = nh.advertise<geometry_msgs::Vector3>("optical_flow", 1000);
